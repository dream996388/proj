...
parser.add_argument('-content', type=str, default='inputs/doodles/Seth.jpg',
...
parser.add_argument('-content_sem', type=str, default='inputs/doodles/Seth_sem.png',
...
parser.add_argument('-out_filename', type=str, default='output.jpg', help="File name of the output image")
...
parser.add_argument('-semantic', type=str, default='concat', choices=['add', 'concat', 'concat_ds'],
                    help="Choose different modes to embed semantic maps, 'add': our addition, 'concat': our concatenation, 'concat_ds': concat downsampled semantic maps")
...
parser.add_argument('-enhance', type=str, default='adain', choices=['adain', 'wct'],
                    help="Choose different enhancement modes in level 3, level 2, and level 1. 'adain': first-order statistics enhancement, 'wct': second-order statistics enhancement.")
...
def main():
    ...
    if args.style_transfer:
        ...
        content = content_sem
    # out_size='heightxwidth'
    args.out_size = f'{content.shape[2:][0]}x{content.shape[2:][1]}'
    # print(args.out_size)
    ...
    print("Processing level 5");
    content = VSTR(TR.e5, TR.d5, content, style, content_sem, style_sem, args.coarse_psize, args.coarse_alpha,
                   args.semantic)

    # Local view texture refinement stage
    print("Processing level 4");
    content = VSTR(TR.e4, TR.d4, content, style, content_sem, style_sem, args.fine_psize, args.fine_alpha, args.semantic)

    # Holistic effect enhancement stage
    print("Processing level 3");
    content = SE(TR.e3, TR.d3, content, style, args.enhance, args.enhance_alpha)
    print("Processing level 2");
    content = SE(TR.e2, TR.d2, content, style, args.enhance, args.enhance_alpha)
    print("Processing level 1");
    content = SE(TR.e1, TR.d1, content, style, args.enhance, args.enhance_alpha)
    ...
    out_path = os.path.join(args.outf, args.out_filename)
    save_image(content.data.cpu(), out_path, out_size=args.out_size, normalize=False)
